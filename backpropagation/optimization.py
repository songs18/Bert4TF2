...
def create_optimizer(loss,tvars, training_config):
    # total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu)
    # init_lr, num_train_steps, num_warmup_steps, use_tpu
    ...
    # learning_rate = tf.keras.optimizers.schedules.PolynomialDecay(learning_rate, global_step, num_train_steps, end_learning_rate=0.0, power=1.0, cycle=False)
...
class AdamWeightDecayOptimizer(tf.keras.optimizers.Optimizer):
    ...
    def __init__(self, learning_rate, weight_decay_rate=0.0, beta_1=0.9, beta_2=0.999, epsilon=1e-6, exclude_from_weight_decay=None,
    ...
    def apply_gradients(self, grads_and_vars, global_step=None, name=None):
        ...
        for (grad, param) in grads_and_vars:
            ...
            m = tf.Variable(name=param_name + "/adam_m", shape=param.shape.as_list(),
                            dtype=tf.float32, trainable=False, initializer=tf.zeros_initializer())
            v = tf.Variable(name=param_name + "/adam_v", shape=param.shape.as_list(),
                            dtype=tf.float32, trainable=False, initializer=tf.zeros_initializer())
